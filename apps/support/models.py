...
Ticketing (``Ticket`` / ``TicketMessage``) is the single customer-support
channel: an asynchronous thread between a customer and staff, optionally
tied to an order and to a return request. A ticket carries a status the
customer's replies and staff actions move it through; every message is
retained so the conversation is fully auditable.
...
from apps.support.constants import TICKET_SUBJECT_MAX_LENGTH
